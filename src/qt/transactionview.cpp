...
void TransactionView::showBrowser()
{
	...
	Q_EMIT blockBrowserSignal(transactionId);
}

void TransactionView::showQRCode()
{
	#ifdef USE_QRCODE
		if(!transactionView->selectionModel())
			return;
	    ...
	    Q_FOREACH (QModelIndex index, selection)
	    {
			...
			QString label = index.sibling(index.row(), 0).data(TransactionTableModel::LabelRole).toString();
