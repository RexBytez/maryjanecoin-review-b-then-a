...

    {
        ...
        else if (wtx.IsCoinStake())
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                if (wallet->IsMine(txout))
                {
                    int64_t nCreditOut = wallet->GetCredit(txout);
                    nActualCredit += nCreditOut;
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, nCreditOut);
