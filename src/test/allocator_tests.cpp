...
BOOST_AUTO_TEST_CASE(test_LockedPageManagerBase)
{
    ...
    BOOST_CHECK(lpm.GetLockedPageCount() == (
        (1000*33+test_page_size-1)/test_page_size +
        101 + 100 +
        501 + 500));
    BOOST_CHECK((last_lock_len & (test_page_size-1)) == 0);
    BOOST_CHECK(last_unlock_len == 0);
    ...
    BOOST_CHECK(lpm.GetLockedPageCount() == 0);
    BOOST_CHECK((last_unlock_len & (test_page_size-1)) == 0);
}
