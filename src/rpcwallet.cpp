...
double GetSupplyChange(int nHeight, int pHeight)
{
	...
	double pSupply = GetMoneySupply(pHeight);
	double nChange = nSupply - pSupply;
	return nChange;
}
...
double GetBlockSpeed(int nHeight, int pHeight)
{
	...
	double nTimeChange = (nTime - pTime) / 60 / 60 / 24;
	return nTimeChange;
}
...
Array printCharity()
{
	Array ret;
	Object act;
	act.push_back(Pair("StakeForCharity Activated?", pwalletMain->fStakeForCharity));
	ret.push_back(act);

	ret.push_back("StakeForCharity Addresses:");
	Object vCharity;
	for(unsigned int i = 0; i < pwalletMain->vStakeForCharity.size(); i++)
	{
		vCharity.push_back(Pair("Address " + boost::lexical_cast<std::string>(i), pwalletMain->vStakeForCharity[i].strAddress));
		vCharity.push_back(Pair("Percent", pwalletMain->vStakeForCharity[i].nPercent));
	}
	ret.push_back(vCharity);

	unsigned int nTotal = 0;
	for(unsigned int i = 0; i < pwalletMain->vStakeForCharity.size(); i++)
		nTotal += pwalletMain->vStakeForCharity[i].nPercent;
	Object total;
	total.push_back(Pair("Total Percent", (int)nTotal));
	ret.push_back(total);

	return ret;
}

unsigned int sumCharity()
{
	unsigned int nTotal = 0;
	for(unsigned int i = 0; i < pwalletMain->vStakeForCharity.size(); i++)
		nTotal += pwalletMain->vStakeForCharity[i].nPercent;
...
Value stakeforcharity(const Array &params, bool fHelp)
{
	...
	if(params.size() == 1)
	{
		...
		if(strCommand == "list" || strCommand == "print")
		{
			return printCharity();
		}
		else if(strCommand == "clear")
		{
			...
			if(fFileBacked)
			{
				if(walletdb.EraseCharity(pwalletMain->vStakeForCharity))
					strRet += "erased charity vector from database & ";
			}
			...
			strRet += "cleared charity vector from RAM";
			return strRet;
		}
		else if (strCommand == "enable" || strCommand == "activate")
		{
			if(pwalletMain->vStakeForCharity.size() < 1)
				return "Unable to activate StakeForCharity, check charity vector";
			if(!CBitcoinAddress(pwalletMain->vStakeForCharity[0].strAddress).IsValid())
				return "Unable to activate StakeForCharity, first address is invalid";

			pwalletMain->fStakeForCharity = true;
			if(fFileBacked)
			{
				if(!walletdb.WriteCharitySettings(true))
					return "StakeForCharity activated but writing settings to DB failed";
			}
			return "StakeForCharity activated";
		}
		else if (strCommand == "disable" || strCommand == "deactivate")
		{
			...
			if(fFileBacked)
			{
				if(!walletdb.WriteCharitySettings(false))
					return "StakeForCharity deactivated but writing settings to DB failed";
			}
			return "StakeForCharity deactivated";
		}
		else if(fHelp)
		{
			throw runtime_error(
				"stakeforcharity <command>\n"
				"****************************************************************\n"
				"WHAT IS STAKE FOR CHARITY?\n"
				"StakeForCharity embeds charity outputs directly into stake transactions\n"
				"Charity portion is sent immediately when you stake, no maturity wait needed\n"
				"You can specify up to 5 addresses with individual percentages\n"
				"Total percentage cannot exceed 100% of stake reward\n"
				"****************************************************************\n"
				"STAKE FOR CHARITY COMMANDS (usage: stakeforcharity <command>)\n"
				"   list/print - displays the current charity vector \n"
				"   clear - deletes the current charity vector \n"
				"   enable/activate - activates the current charity vector \n"
				"   disable/deactivate - disables the current charity vector \n"
				"   add <address> <percent> - adds an address to the charity vector (max 5)\n"
				"   remove <address> - removes an address from the charity vector \n"
				"\n"
				"LEGACY FORMAT (still supported):\n"
				"stakeforcharity <MaryJaneCoin Address> <percent>\n"
				"Clears existing entries and adds single address\n"
				"Set percent to 0 to disable\n"
				"****************************************************************\n"
				);
		}
		else
		{
			return "Invalid command. Use 'stakeforcharity' for help.";
		}
	}

	if(params.size() == 2 && params[0].get_str() == "remove")
	{
		if (pwalletMain->IsLocked())
			throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");
		...
		if(!CBitcoinAddress(strAddress).IsValid())
			return "Address you want to remove is not valid";

		LOCK(pwalletMain->cs_wallet);
		bool fRemoved = false;
		std::vector<StakeForCharityEntry> vCharityTemp = pwalletMain->vStakeForCharity;

		for(unsigned int i = 0; i < pwalletMain->vStakeForCharity.size(); i++)
		{
			if(pwalletMain->vStakeForCharity[i].strAddress == strAddress)
			{
				pwalletMain->vStakeForCharity.erase(pwalletMain->vStakeForCharity.begin() + i);
				fRemoved = true;
				break;
			}
		}
		...
		if(!fRemoved)
			return "Could not locate address in charity vector";

		if(fFileBacked)
		{
			if(!walletdb.EraseCharity(vCharityTemp))
				return "Failed to erase old charity vector from database";
			if(!walletdb.WriteCharity(pwalletMain->vStakeForCharity))
				return "Failed to write new charity vector to database";
		}

		return printCharity();
	}

	if(params.size() == 3 && params[0].get_str() == "add")
	{
		if (pwalletMain->IsLocked())
			throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");
		...
		unsigned int nPercent = 0;
		try {
			nPercent = boost::lexical_cast<unsigned int>(params[2].get_str());
		} catch (...) {
		...
		if(pwalletMain->vStakeForCharity.size() >= 5)
			return "Failed to add to charity vector, maximum 5 addresses supported";

		for(unsigned int i = 0; i < pwalletMain->vStakeForCharity.size(); i++)
		{
			if(pwalletMain->vStakeForCharity[i].strAddress == strAddress)
				return "Failed to add to charity vector, cannot use the same address twice";
		}

		if (nPercent + sumCharity() > 100)
			return "Failed to add to charity vector, the sum of your charity percentages would be greater than 100%";
		...
		StakeForCharityEntry entry(strAddress, nPercent);
		pwalletMain->vStakeForCharity.push_back(entry);

		if(fFileBacked)
		{
			if(!walletdb.WriteCharity(pwalletMain->vStakeForCharity))
				return "Failed to write charity vector to database";
		}
	...
	if(params.size() == 2)
	{
		...
		unsigned int nPer = 0;
		try {
			nPer = boost::lexical_cast<unsigned int>(params[1].get_str());
		} catch (...) {
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, expected valid percentage");
		}
		...
		if(nPer == 0)
		{
			...
			{
				walletdb.WriteCharitySettings(false);
				walletdb.EraseCharity(vCharityTemp);
			}
			return Value::null;
		}
		...
		std::vector<StakeForCharityEntry> vCharityTemp = pwalletMain->vStakeForCharity;
		pwalletMain->vStakeForCharity.clear();
		StakeForCharityEntry entry(address.ToString(), nPer);
		pwalletMain->vStakeForCharity.push_back(entry);
		pwalletMain->fStakeForCharity = true;
		...
		if(fFileBacked)
		{
			if(!walletdb.EraseCharity(vCharityTemp))
				return "Failed to erase old charity vector from database";
			if(!walletdb.WriteCharitySettings(true))
				throw JSONRPCError(RPC_WALLET_ERROR, "Error: Failed to save charity settings to database");
			if(!walletdb.WriteCharity(pwalletMain->vStakeForCharity))
				throw JSONRPCError(RPC_WALLET_ERROR, "Error: Failed to save charity addresses to database");
		}
	...
	throw runtime_error("Invalid parameters. Use 'stakeforcharity' for help.");
}
...
Value checkwallet(const Array& params, bool fHelp)
{
    ...
    int64_t nConflictedBalance = pwalletMain->GetConflictedBalance();
    Object result;
    if (nMismatchSpent == 0 && nOrphansFound == 0 && nConflictedBalance == 0)
    ...
    else
    {
        ...
        result.push_back(Pair("conflicted balance", ValueFromAmount(nConflictedBalance)));
    }
    return result;
}
...
Value repairwallet(const Array& params, bool fHelp)
{
    ...
    const int stderr_backup_fd = dup(stderr_fd);
    ...
    if (stderr_backup_fd >= 0) {
        fflush(stderr);
        fclose(stderr);
        dup2(stderr_backup_fd, stderr_fd);
    ...
    else
    {
        ...
        if (nConflictedBalance != 0)
            result.push_back(Pair("conflicted balance remaining", ValueFromAmount(nConflictedBalance)));
    }
    return result;
}
...
Value getstaketx(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain->mapWallet.count(hash))
    {
		 ...
		 BOOST_FOREACH(const CTxIn& txin, wtx.vin)
		{
			...
			{
				CTransaction& txPrev = pwalletMain->mapWallet[txin.prevout.hash];
				uint64_t nTime = wtx.nTime;
				uint64_t nPrevTime = txPrev.nTime;
				uint64_t nTimeToStake = nTime - nPrevTime;
				...
				double nGeneratedAmount2 = max (nGeneratedMature, nGeneratedImmature);
...
Value cccustomchange(const Array& params, bool fHelp)
{
    ...
    if(!address.IsValid()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid MaryJaneCoin address");
...
Value ccsend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
		"ccsend <MaryJaneCoin Address> <amount>\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
   ...
   vector<pair<CScript, int64_t> > vecSend;
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, 1, coinControl);
    if (!fCreated)
    {
        if (nAmount + nFeeRequired > pwalletMain->GetBalance())
            throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Insufficient funds");
...
Array printAddresses()
{
	...
	BOOST_FOREACH(const COutput& out, vCoins)
    {
		...
		if(mapAddresses.find(strAdd) == mapAddresses.end())
		{
...
Value multisend(const Array &params, bool fHelp)
{
	...
	if (fHelp || params.size() != 2)
        throw runtime_error(
			...
			"multisend <MaryJaneCoin Address> <percent>\n"
