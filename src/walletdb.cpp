             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
		...
		else if(strType == "hashdrift")
		{
		   unsigned int nHashDrift;
		   ssValue >> nHashDrift;
		   pwallet->nHashDrift = nHashDrift;
		}
		else if(strType == "hashinterval")
		{
		   unsigned int nHashInterval;
		   ssValue >> nHashInterval;
		   pwallet->nHashInterval = nHashInterval;
		}
		else if(strType == "charity")
		{
			unsigned int i;
			ssKey >> i;
			StakeForCharityEntry charityEntry;
			ssValue >> charityEntry;
			if(CBitcoinAddress(charityEntry.strAddress).IsValid() && charityEntry.nPercent > 0 && charityEntry.nPercent <= 100)
			{
				pwallet->vStakeForCharity.push_back(charityEntry);
			}
		}
		else if(strType == "charitysettings")
		{
			bool fEnable;
			ssValue >> fEnable;
    ...
    try {
        ...
        while (true)
        {
            ...
            if (!ReadKeyValue(pwallet, ssKey, ssValue, wss, strType, strErr))
            {
                ...
                {
                    printf("LoadWallet: Key type error - marking DB_CORRUPT\n");
                    result = DB_CORRUPT;
                }
                else
                {
                    ...
                    printf("LoadWallet: Non-critical error for type=%s\n", strType.c_str());
                    if (strType == "tx")
