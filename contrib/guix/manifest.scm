...
;; Clang 18: Define at top level for use throughout manifest
;; This ensures stable package identities and prevents unnecessary rebuilds
(define clang-toolchain-18 (@ (gnu packages llvm) clang-toolchain-18))
(define lld-18 (@ (gnu packages llvm) lld-18))
(define darwin-lld-wrapper (make-lld-wrapper lld-18 #:lld-as-ld? #t))

(define* (make-maryjanecoin-cross-toolchain target
                                       #:key
...
desirable for building MaryJaneCoin release binaries."
...
(define osslsigncode
  (package
    ...
    (inputs (list openssl))
    (arguments '(#:tests? #f)) ; Disable tests - test certificates have expired
    ...
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "-linux-")
           (list bison
                 ...
                 (make-maryjanecoin-cross-toolchain target)))
